...
from logSystemApp.serializers_module.serializers import LogSerializer, ErrorLogSerializer
...
class ErrorLogView(APIView):
    ...
    def get(self, request):
        ...
        if user_id_query:
            list_obj = ErrorLog.objects.filter(user=user_id_query)
        else:
            list_obj =  ErrorLog.objects.all()
...
class ErrorLogViewDeleteAll(APIView):
    permission_classes = [IsStaffOrSuperUser]
    def delete(self, request):
        if not request.user.is_superuser and not request.user.has_perm('usersAuthApp.logs_delete'):
            return Response({"detail": "Permission denied for this operation."}, status=status.HTTP_403_FORBIDDEN)
