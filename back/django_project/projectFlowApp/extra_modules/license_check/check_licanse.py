...
from projectFlowApp.extra_modules.license_check.get_device_app_fingerprint import generate_device_app_fingerprint
...
class LicenseChecker:
    ...
    def __init__(
        self,
        app_name='projectflow',   # <-- app name here
        # license_path = '../licenses/license.json',

        license_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), '..', '..', 'licenses', 'license.json')
        ),
        ...
        last_check_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), 'last_license_check.json')
        ),
        ...
        self.machine_id_obj = generate_device_app_fingerprint()
    ...
    def _load_public_key(self):
        try:
            ...
            raise RuntimeError(f"Failed to load public key: {e}" , self.public_key_path)
    ...
    def _build_license_response(self, is_valid, reason=None):
        return {
            'license_id': self._license_data.get('license_id'),
            'device_app_fingerprint': self._license_data.get('device_app_fingerprint'),
            ...
            'app_installation_id' : self._license_data.get('app_installation_id'),
            'app_Name_id' : self._license_data.get('app_Name_id'),
        ...
        with self._lock:
            ...
            if cached_result:
                ...
                if now - cached_time < timedelta(seconds=self.cache_timeout):
                    ...
                    if now < last_check['timestamp'] - self.GRACE_PERIOD:
            ...
            last_check = self._check_last_check_file()
            if now + self.GRACE_PERIOD < last_check['timestamp']:
                self._save_last_check_file(now, False)
                license_response = self._build_license_response(False,
                       'System clock rollback detected - license invalidated222'
                    )
                self._cache_license_result(now, license_response)
                return license_response

            if self._license_data['device_app_fingerprint'] != self.machine_id_obj['device_app_fingerprint']:
                self._save_last_check_file(now, False)

                license_response = self._build_license_response(False,
                       'device_app_fingerprint not valid'
                    )
